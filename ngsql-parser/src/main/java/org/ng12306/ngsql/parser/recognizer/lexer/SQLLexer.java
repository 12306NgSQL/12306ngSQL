...
public class SQLLexer {
    ...
    /** make sense only for {@link Token#IDENTIFIER} */
     ...
     */
    public final void addCacheToke(Token token) {
        tokenCache = token;
    }
    ...
    private Token nextTokenInternal() throws SQLSyntaxErrorException {
        switch (ch) {
        case '0':
        ...
        case '[':
            scanChar();
            token = Token.PUNC_LEFT_BRACKET;
            return token;
        case ']':
            scanChar();
            token = Token.PUNC_RIGHT_BRACKET;
            return token;
        case '{':
            scanChar();
            token = Token.PUNC_LEFT_BRACE;
            return token;
        case '}':
            scanChar();
            token = Token.PUNC_RIGHT_BRACE;
            return token;
        case ',':
            scanChar();
        ...
        case '*':
            if (inCStyleComment && sql[curIndex + 1] == '/') {
                ...
                scanChar(2);
                token = Token.PUNC_C_STYLE_COMMENT_END;
                return token;
            }
            scanChar();
        ...
        case '<':
            ...
            case '=':
                if (sql[curIndex + 2] == '>') {
                    scanChar(3);
                    token = Token.OP_NULL_SAFE_EQUALS;
                    return token;
                }
                scanChar(2);
                token = Token.OP_LESS_OR_EQUALS;
                return token;
            case '>':
                scanChar(2);
                token = Token.OP_LESS_OR_GREATER;
                return token;
            case '<':
                scanChar(2);
                token = Token.OP_LEFT_SHIFT;
                return token;
            default:
                scanChar();
    ...
    public Token nextToken() throws SQLSyntaxErrorException {
        if (tokenCache != null) {
            tokenCache = null;
            return token;
        }
        ...
        do {
            skipSeparator();
            t = nextTokenInternal();
        } while (inCStyleComment && inCStyleCommentIgnore || Token.PUNC_C_STYLE_COMMENT_END == t);
        return t;
    }
    ...
    /**
     * if first char is <code>.</code>, token may be {@link Token#PUNC_DOT} if
            ...
            case 3:
                if (CharTypes.isDigit(ch)) {
                    ...
                    if (CharTypes.isIdentifierChar(ch)) {
                        ...
                        updateStringValue(sql, offsetCache, sizeCache);
                        Token tok = keywods.getKeyword(stringValueUppercase);
                        token = tok == null ? Token.IDENTIFIER : tok;
                    }
                    return;
                } else {
    ...
    /**
     * NOTE: {@link Token#IDENTIFIER id} dosn't include <code>'.'</code> for
     * sake of performance issue (based on <i>shaojin.wensj</i>'s design).
    ...
    /**
     * {@link #token} must be {@link Token#LITERAL_NUM_PURE_DIGIT}
     */
    ...
    /**
     * make sense for those types of token:<br/>{@link Token#USR_VAR}: e.g. "@var1",
     * "@'mary''s'";<br/> {@link Token#SYS_VAR}: e.g. "var2";<br/>
     * {@link Token#LITERAL_CHARS}, {@link Token#LITERAL_NCHARS}: e.g.
     * "'ab\\'c'";<br/> {@link Token#LITERAL_BIT}: e.g.
     * "0101" <br/>{@link Token#IDENTIFIER}
     */
    ...
    /**
     * for {@link Token#IDENTIFIER}, {@link Token#SYS_VAR}
     */
